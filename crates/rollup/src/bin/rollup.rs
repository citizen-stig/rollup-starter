...
async fn new_rollup(
    ...
    let evm_pinned_cache_config = rollup_config_path.parent().unwrap_or_else(|| panic!("Provided rollup config path {} does not have a parent directory", rollup_config_path.display())).join("evm_pinned_cache.json");
